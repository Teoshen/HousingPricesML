...
## Section 3: Dealing with Missing Data (5 points)
...
```{r}
train_set <- train_set %>%
  mutate(TotalSF = sum(train_set$GrLivArea, train_set$BsmtFinSF1))

test_set <- test_set %>%
  mutate(TotalSF = sum(test_set$GrLivArea, test_set$BsmtFinSF1))
```

Simplified condition/grade combination

```{r}
train_set <- train_set %>%
  mutate(OverallScore = OverallQual * OverallCond)
...
```{r}
train_glm <- glm(SalePrice ~ MSZoning + MSSubClass + LotFrontage +  LotArea + Street + LotShape + LandContour + LotConfig + LandSlope + Neighborhood + Condition1+ Condition2 + BldgType + HouseStyle + OverallQual + OverallCond + YearBuilt + YearRemodAdd + RoofStyle + RoofMatl + Exterior1st + Exterior2nd + MasVnrType + MasVnrArea + ExterQual + ExterCond + Foundation + BsmtQual + BsmtCond + BsmtExposure + BsmtFinType1 + BsmtFinSF1 + BsmtFinType2 + BsmtFinSF2 + BsmtUnfSF + TotalBsmtSF + Heating + HeatingQC + CentralAir + Electrical + X1stFlrSF + X2ndFlrSF+ LowQualFinSF + GrLivArea + BsmtFullBath + BsmtHalfBath + FullBath + HalfBath + BedroomAbvGr + KitchenAbvGr + KitchenQual+ TotRmsAbvGrd + Functional + Fireplaces + GarageType + GarageYrBlt + GarageFinish + GarageCars + GarageArea + PavedDrive + WoodDeckSF + OpenPorchSF + EnclosedPorch + X3SsnPorch + ScreenPorch + PoolArea + Fence + MiscVal + MoSold + YrSold + SaleType + SaleCondition, data = train_set, family = gaussian)

stepAIC(train_glm, direction = "both")
```

```         
MSZoning + MSSubClass + LotFrontage + LotArea + Street + 
    LandContour + LotConfig + LandSlope + Neighborhood + Condition1 + 
    Condition2 + OverallQual + OverallCond + YearBuilt + YearRemodAdd + 
    RoofMatl + MasVnrType + MasVnrArea + ExterQual + BsmtExposure + 
    BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF + X1stFlrSF + X2ndFlrSF + 
    BsmtFullBath + BedroomAbvGr + KitchenAbvGr + KitchenQual + 
    TotRmsAbvGrd + Functional + GarageArea + WoodDeckSF + OpenPorchSF + 
    EnclosedPorch + X3SsnPorch + ScreenPorch + MoSold + SaleCondition
```
...
```{r}
train_knn <- train(SalePrice  ~ MSZoning + MSSubClass + LotFrontage + LotArea + Street + 
    LandContour + LotConfig + LandSlope + Neighborhood + Condition1 + 
    Condition2 + OverallQual + OverallCond + YearBuilt + YearRemodAdd + 
    RoofMatl + MasVnrType + MasVnrArea + ExterQual + BsmtExposure + 
    BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF + X1stFlrSF + X2ndFlrSF + 
    BsmtFullBath + BedroomAbvGr + KitchenAbvGr + KitchenQual + 
    TotRmsAbvGrd + Functional + GarageArea + WoodDeckSF + OpenPorchSF + 
    EnclosedPorch + X3SsnPorch + ScreenPorch + MoSold + SaleCondition, method = "knn", data = train_set, tuneGrid = data.frame(k = seq(9, 71, 2)))
...
```{r}
train_dt <- train(SalePrice ~ MSZoning + MSSubClass + LotFrontage + LotArea + Street + 
    LandContour + LotConfig + LandSlope + Neighborhood + Condition1 + 
    Condition2 + OverallQual + OverallCond + YearBuilt + YearRemodAdd + 
    RoofMatl + MasVnrType + MasVnrArea + ExterQual + BsmtExposure + 
    BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF + X1stFlrSF + X2ndFlrSF + 
    BsmtFullBath + BedroomAbvGr + KitchenAbvGr + KitchenQual + 
    TotRmsAbvGrd + Functional + GarageArea + WoodDeckSF + OpenPorchSF + 
    EnclosedPorch + X3SsnPorch + ScreenPorch + MoSold + SaleCondition, method = "rpart", tuneGrid = data.frame(cp=seq(0,0.05, len = 25)), data = train_set)
...
```{r}
train_rf <- train(SalePrice ~ MSZoning + MSSubClass + LotFrontage + LotArea + Street + 
    LandContour + LotConfig + LandSlope + Neighborhood + Condition1 + 
    Condition2 + OverallQual + OverallCond + YearBuilt + YearRemodAdd + 
    RoofMatl + MasVnrType + MasVnrArea + ExterQual + BsmtExposure + 
    BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF + X1stFlrSF + X2ndFlrSF + 
    BsmtFullBath + BedroomAbvGr + KitchenAbvGr + KitchenQual + 
    TotRmsAbvGrd + Functional + GarageArea + WoodDeckSF + OpenPorchSF + 
    EnclosedPorch + X3SsnPorch + ScreenPorch + MoSold + SaleCondition, method = "rf", tuneGrid = expand.grid(mtry = c(3,5)), data = train_set)
```
...
```{r}
train_xgb <- train(SalePrice ~ MSZoning + MSSubClass + LotFrontage + LotArea + Street + 
    LandContour + LotConfig + LandSlope + Neighborhood + Condition1 + 
    Condition2 + OverallQual + OverallCond + YearBuilt + YearRemodAdd + 
    RoofMatl + MasVnrType + MasVnrArea + ExterQual + BsmtExposure + 
    BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF + X1stFlrSF + X2ndFlrSF + 
    BsmtFullBath + BedroomAbvGr + KitchenAbvGr + KitchenQual + 
    TotRmsAbvGrd + Functional + GarageArea + WoodDeckSF + OpenPorchSF + 
    EnclosedPorch + X3SsnPorch + ScreenPorch + MoSold + SaleCondition, data = train_set, method = "xgbTree", trControl = trainControl(method = "cv", number = 5, verboseIter = TRUE), tuneGrid = expand.grid(nrounds = c(100, 200), max_depth = c(3,6,9), eta = c(0.01, 0.1, 0.3), gamma = c(0, 1), colsample_bytree = c(0.6, 0.8, 1.0), min_child_weight = c(1,5), subsample = c(0.6, 0.8, 1.0)))
...
```{r}
test_glm <- train(SalePrice ~ MSZoning + MSSubClass + LotFrontage + LotArea + Street + 
    LandContour + LotConfig + LandSlope + Neighborhood + Condition1 + 
    Condition2 + OverallQual + OverallCond + YearBuilt + YearRemodAdd + 
    RoofMatl + MasVnrType + MasVnrArea + ExterQual + BsmtExposure + 
    BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF + X1stFlrSF + X2ndFlrSF + 
    BsmtFullBath + BedroomAbvGr + KitchenAbvGr + KitchenQual + 
    TotRmsAbvGrd + Functional + GarageArea + WoodDeckSF + OpenPorchSF + 
    EnclosedPorch + X3SsnPorch + ScreenPorch + MoSold + SaleCondition, family = gaussian, data = train_base)
...
```{r}
test_knn <- train(SalePrice ~ MSZoning + LotFrontage + LotArea +      Street + Neighborhood + Condition1 + OverallQual + OverallCond +      YearBuilt + YearRemodAdd + RoofMatl + Exterior1st + Exterior2nd +      BsmtQual + BsmtCond + BsmtExposure + BsmtFinType1 + BsmtFinSF1 +      BsmtFinSF2 + BsmtUnfSF + X1stFlrSF + X2ndFlrSF + LowQualFinSF +      KitchenAbvGr + TotRmsAbvGrd + Functional + Fireplaces + GarageFinish +      GarageCars + WoodDeckSF + X3SsnPorch + ScreenPorch + SaleCondition, data = train_base, method = "knn", tuneGrid = data.frame(k = 11))
...
```{r}
test_dt <- train(SalePrice ~ MSZoning + LotFrontage + LotArea +      Street + Neighborhood + Condition1 + OverallQual + OverallCond +      YearBuilt + YearRemodAdd + RoofMatl + Exterior1st + Exterior2nd +      BsmtQual + BsmtCond + BsmtExposure + BsmtFinType1 + BsmtFinSF1 +      BsmtFinSF2 + BsmtUnfSF + X1stFlrSF + X2ndFlrSF + LowQualFinSF +      KitchenAbvGr + TotRmsAbvGrd + Functional + Fireplaces + GarageFinish +      GarageCars + WoodDeckSF + X3SsnPorch + ScreenPorch + SaleCondition, method = "rpart", tuneGrid = data.frame(cp= 0.01), data = train_base)
...
```{r}
test_rf <- train(SalePrice ~ MSZoning + MSSubClass + LotFrontage + LotArea + Street + 
    LandContour + LotConfig + LandSlope + Neighborhood + Condition1 + 
    Condition2 + OverallQual + OverallCond + YearBuilt + YearRemodAdd + 
    RoofMatl + MasVnrType + MasVnrArea + ExterQual + BsmtExposure + 
    BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF + X1stFlrSF + X2ndFlrSF + 
    BsmtFullBath + BedroomAbvGr + KitchenAbvGr + KitchenQual + 
    TotRmsAbvGrd + Functional + GarageArea + WoodDeckSF + OpenPorchSF + 
    EnclosedPorch + X3SsnPorch + ScreenPorch + MoSold + SaleCondition, method = "rf", tuneGrid = expand.grid(mtry = 5), data = train_base)
...
```{r}
test_xgb <- train(SalePrice ~ MSZoning + MSSubClass + LotFrontage + LotArea + Street + 
    LandContour + LotConfig + LandSlope + Neighborhood + Condition1 + 
    Condition2 + OverallQual + OverallCond + YearBuilt + YearRemodAdd + 
    RoofMatl + MasVnrType + MasVnrArea + ExterQual + BsmtExposure + 
    BsmtFinSF1 + BsmtFinSF2 + BsmtUnfSF + X1stFlrSF + X2ndFlrSF + 
    BsmtFullBath + BedroomAbvGr + KitchenAbvGr + KitchenQual + 
    TotRmsAbvGrd + Functional + GarageArea + WoodDeckSF + OpenPorchSF + 
    EnclosedPorch + X3SsnPorch + ScreenPorch + MoSold + SaleCondition, data = train_base, method = "xgbTree", trControl = trainControl(method = "cv", number = 5), tuneGrid = expand.grid(nrounds = 200, max_depth = 9, eta = 0.1, gamma = 0, colsample_bytree = .08, min_child_weight = 1, subsample = 0.8))
...
## Section 7: Ensembles (10 points)

For our ensemble, we'll use GLM, RF, and XGB, our best performers.

```{r}
saleprice_ensemble <- rowMeans(cbind(saleprice_test_glm, saleprice_test_rf, saleprice_test_xgb))


submission_ensemble <- test_base %>%   
  select(Id) %>%   
  mutate(SalePrice = saleprice_ensemble) 

write.csv(submission_ensemble, 'submission_ensemble.csv', row.names = FALSE)
```

19927.57566. Only ever so slightly worse than GLM by itself.

## Section 8: Prediction (write your solution to a csv file) (5 points)
...
Submitted with a score of 19915.46909. That seems pretty good!
...
41473.38284. Not that great, maybe my neighbors are off.
...
22464.60598. I am noticing that GLM and RF tend to perform pretty
similarly.
...
20666.83323. First time using this one and I think I like it except for
the training time compute. I'm curious why it did so well in training
and then not so well here, maybe overfitting.

This places me at position #3322 on the leaderboard as of the time of
